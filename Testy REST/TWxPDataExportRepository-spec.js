...
frisby.create('Create directory \"test\" in root folder of repository.')
	.get('http://' + logon + config.Thing + 'TWxPDataExportRepository/Services/CreateFolder?method=post&path=/test&Accept=application/json')
	.expectStatus(200)
	.toss();

frisby.create('Check if directory \"test\" exist.')
	.get('http://' + logon + config.Thing + 'TWxPDataExportRepository/Services/BrowseDirectory?method=post&path=/&Accept=application/json')
	.expectJSON("rows.?", {
	"fileType": "D",
	"name": "test",
	"path": "/test"
...
frisby.create('Delete directory \"test\".')
	.get('http://' + logon + config.Thing + 'TWxPDataExportRepository/Services/DeleteFolder?method=post&path=/test&Accept=application/json')
	.expectStatus(200)
	.toss();
