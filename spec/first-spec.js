...
var logger = new (winston.Logger)({
	transports:[
		new (winston.transports.File)({name: 'first-info-file', filename: './first-info.log', level: 'info' }),
		new (winston.transports.File)({name: 'first-error-file', filename: './first-error.log', level: 'error' })]
});
	...
	it('Check if you can fill form and click button', function(done){
		...
		.waitFor('#root_ContainedMashup-9_Button-8 button', 7000, function(err){
			...
			browser.click('#root_ContainedMashup-9_Button-8 button', function(err){
				//poczekaj na załadowanie komunikatu że plik o nazwie 'test123' nie istnieje w repozytorium
